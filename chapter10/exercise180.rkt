...
#reader(lib "htdp-beginner-reader.ss" "lang")((modname exercise180) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
(require 2htdp/image)
...
(check-expect (my-implode (cons "a"
                                (cons "b"
                                      (cons "c" '()))))
              "abc")
(define (my-implode alos)
  (cond
    [(empty? alos) ""]
    [else (string-append
           (first alos)
           (my-implode (rest alos)))]))
