...
#reader(lib "htdp-beginner-abbr-reader.ss" "lang")((modname exercise204) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
...
; produces a list of LTracks where each list
; is all of the tracks in an album
(check-expect (select-albums (list "Parachutes") ex-list-tracks)
              (list (list extrack2)))
(check-expect (select-albums (list "Parachutes" "track album") ex-list-tracks)
              (list (list extrack2) (list extrack1)))
(define (select-albums albums tracks)
  (cond
    [(= (length albums) 1) (list (select-album (first albums) tracks))]
    [else (cons (select-album (first albums) tracks) (select-albums (rest albums) tracks))]))