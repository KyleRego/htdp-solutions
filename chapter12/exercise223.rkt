...
#reader(lib "htdp-beginner-abbr-reader.ss" "lang")((modname exercise223) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
...
; Tetris -> Boolean
; determines if one of the stacks has reached the top
; returns true if so
(define (stopper tet)
  (cond
    [(empty? (tetris-landscape tet)) #false]
    [else (or
           (= (block-y (first (tetris-landscape tet))) 0)
           (stopper (make-tetris (tetris-block tet)
                                 (rest (tetris-landscape tet)))))]))
